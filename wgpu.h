...
WGPUBuffer frmwrk_device_create_buffer_init(WGPUDevice device, const frmwrk_buffer_init_descriptor *descriptor) {
  assert(descriptor);
  ...
  size_t padded_size = MAX((unpadded_size + (COPY_BUFFER_ALIGNMENT - 1)) & ~(COPY_BUFFER_ALIGNMENT - 1), COPY_BUFFER_ALIGNMENT);
...
void frmwrk_print_global_report(WGPUGlobalReport report) {
  ...
  switch (report.backendType) {
    case WGPUBackendType_D3D12: print_hub_report(report.dx12, "\tdx12."); break;
    case WGPUBackendType_Metal: print_hub_report(report.metal, "\tmetal."); break;
    case WGPUBackendType_Vulkan: print_hub_report(report.vulkan, "\tvulkan."); break;
    case WGPUBackendType_OpenGL: print_hub_report(report.gl, "\tgl."); break;
    default: printf("[framework] frmwrk_print_global_report: invalid backend type: %d", report.backendType);
  }
  printf("}\n");
}
